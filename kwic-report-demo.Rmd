...
Then, use `write.csv()` to save the `kwic_df_by_keyword` to the given file name.
...
Adding the `include_stats = TRUE` option will produce a second series of dataframes that
provide an overview of the output, including summative information about the
number of terms searched, number of documents (in this case, individual finding aids),
and other information. To save this information, run with the `include_stats` option,
then use an approach like the one illustrated below to save the additional "stats":
